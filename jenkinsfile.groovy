pipeline {
  agent {
    docker {
      image 'krishna2710/javaapplication-docker-agent:v1'
  ...
  stages {
    ...
    stage('Build and Test') {
      steps {
        ...
        sh 'cd javaapplication && mvn clean package'
      }
    }
    stage('Static Code Analysis') {
      environment {
        SONAR_URL = "http://44.201.100.206:9000"
      }
      steps {
        withCredentials([string(credentialsId: 'sonarqube', variable: 'SONAR_AUTH_TOKEN')]) {
          sh 'cd javaapplication && mvn sonar:sonar -Dsonar.login=$SONAR_AUTH_TOKEN -Dsonar.host.url=${SONAR_URL}'
        }
      }
    }
    stage('Build and Push Docker Image') {
      ...
      steps {
        script {
            sh 'cd javaapplication && docker build -t ${DOCKER_IMAGE} .'
